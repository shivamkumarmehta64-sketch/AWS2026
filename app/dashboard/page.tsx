...
import { SkiperDynamicDock } from '@/components/ui/SkiperUI';
import { AnimMasterCanvas } from '@/components/ui/AnimMasterCanvas';
import { MapPin, BarChart3, Activity, Cpu, QrCode, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function GovernmentAWSManagementPortal() {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-all duration-300 relative overflow-hidden ${isMissionControl ? 'bg-slate-950 text-slate-100' : 'bg-[#0B132B] text-slate-100'}`}>
      {/* Dynamic Master Canvas Background */}
      <AnimMasterCanvas colorScheme="sky" spacing={isMissionControl ? 48 : 36} interactiveRadius={140} />
        ...
        <div className="max-w-[1750px] mx-auto px-4 lg:px-8 flex items-center justify-between overflow-x-auto hide-scrollbar">
          ...
          <div className="hidden lg:flex items-center gap-2 pl-4 border-l border-slate-800/80">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-400/10 hover:bg-amber-400/20 text-amber-300 border border-amber-400/30 text-xs font-bold transition-all cursor-pointer"
            >
              <RefreshCw className="w-3.5 h-3.5" /> Replay Dataset
            </button>
            <button
              onClick={() => setIsMobileQROpen(true)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-400/10 hover:bg-cyan-400/20 text-cyan-300 border border-cyan-400/30 text-xs font-bold transition-all cursor-pointer"
            >
              <QrCode className="w-3.5 h-3.5" /> Mobile Sensor
            </button>
            <button
              onClick={() => setIsEmergencyAlertOpen(true)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-bold transition-all cursor-pointer"
            >
              <AlertTriangle className="w-3.5 h-3.5" /> CAP Broadcast
            </button>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 max-w-[1750px] w-full mx-auto px-4 lg:px-8 py-6 space-y-6">
        ...
        {/* Automated Live India Citizen & Sensor Test */}
        ...
        {/* Universal All-India District & City Search */}
      ...
      {/* Floating Dynamic Action Dock */}
      <SkiperDynamicDock
        items={[
          { id: 'map', label: 'Live Map', icon: <MapPin className="w-5 h-5" />, active: activeTab === 'map', onClick: () => setActiveTab('map') },
          { id: 'analytics', label: 'Vayu Analytics', icon: <BarChart3 className="w-5 h-5" />, active: activeTab === 'analytics', onClick: () => setActiveTab('analytics') },
          { id: 'diagnostics', label: 'Diagnostics', icon: <Activity className="w-5 h-5" />, active: activeTab === 'diagnostics', onClick: () => setActiveTab('diagnostics') },
          { id: 'simulator', label: 'Simulator', icon: <Cpu className="w-5 h-5" />, active: activeTab === 'simulator', onClick: () => setActiveTab('simulator') },
          { id: 'qr', label: 'Mobile Sensor', icon: <QrCode className="w-5 h-5" />, badge: connectedMobileCount > 0 ? connectedMobileCount : undefined, onClick: () => setIsMobileQROpen(true) },
          { id: 'alert', label: 'Emergency Alert', icon: <AlertTriangle className="w-5 h-5" />, onClick: () => setIsEmergencyAlertOpen(true) },
        ]}
      />

      {/* Footer */}
      <footer className="mt-8 bg-slate-950 text-slate-300 border-t-4 border-[#FF9933] text-xs">
        <div className="max-w-[1750px] mx-auto px-4 lg:px-8 py-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pb-6 border-b border-slate-800">
            ...
            <div>
              ...
              <button onClick={() => setActiveModal('architecture')} className="mt-2 text-sky-300 hover:text-white underline text-[11px] font-semibold cursor-pointer">
          ...
          <div className="text-[10px] text-slate-400 border-t border-slate-800/80 pt-2 text-center">
